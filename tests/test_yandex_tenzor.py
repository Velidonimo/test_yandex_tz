...
class TestYandexSearch(Base):
    ...
    def test_open_yandex_image(self):
        ...
        with allure.step("Compare the clicked image to the opened one"):
            if not (page_images.compare_image_text(image_text)):
                raise AssertionError(f'The opened image title is not compared to the clicked one: "{image_text}"')
